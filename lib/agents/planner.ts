...
//         console.warn(` ${validation.warnings.length} warnings fixed`);
//       }
...
export async function PlannerAgent(userQuery: string): Promise<PlannerOutput> {
  ...
  const missingDependencies = filterMissingDependencies(plan.npm_dependencies);

  console.log(` ${plan.features.length} features planned`);
...
function filterMissingDependencies(requested: string[]) {
  return requested.filter((dep) => {
    const name = dep.split("@")[0];
